class CodeLinesEngine
{
	...
	manageError(lang)
	{
		...
		if (codeResult)
		{
			...
			{
				error = lang == "en" ? "Missing a point between the name of the object and the desired action" : "オブジェクトのなまえともくてきのあくしょんのまにぽいんとがない";
			}
			else
			{
				error = lang == "en" ? "The action attributed to the object is non-existent or incorrect. Correct code example: " + codeResult[0] + "." + "moveFront(1)" : "みかくにんのじょうほうがありません。こうどれいのしゅうせい" + codeResult[0] + "." + "moveFront(1)";
			}
		}
		...
		else
		{
			...
			if (codeUpperResult)
			{
				error = lang == "en" ? "A capital letter is missing in the object namen: \"" + codeUpperResult[0] + "\"" : "オブジェクトめいにだいもじがありません: \"" + codeUpperResult[0] + "\"";
			}
			else
			{
				// Kono obujekuto no namae matawa yōkyū sa reta akushon wa sonzai shimasen
				error = lang == "en" ? "The name of this object or the requested action does not exist!" : "このオブジェクトのなまえまたわようきゅされたあくしょんはそんわいしません";
			}
		}
		return error;
	}
